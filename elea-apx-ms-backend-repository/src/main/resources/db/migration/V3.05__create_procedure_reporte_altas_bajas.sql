CREATE PROCEDURE [dbo].[Reporte_Aprobacion_Altas_Bajas] @ciclo int
AS
BEGIN
	SELECT
	...
	JOIN motivo mt ON (mt.id = cme.motivo_estado_id )
	JOIN datos_visita dv ON dv.id=cm.datos_visita_id
	JOIN direccion d2 ON d2.id=dv.id_direccion
	...
	JOIN calle c2 ON c2.id = cp.id_ciudad
	JOIN usuario u2 ON u2.id=cme.aprobador_id
	INNER JOIN [ciclo] ci ON ci.id = @ciclo
	WHERE cme.estado IN ('ALTA_APROBADA', 'BAJA_APROBADA')
	AND cm.inactivo = 0
