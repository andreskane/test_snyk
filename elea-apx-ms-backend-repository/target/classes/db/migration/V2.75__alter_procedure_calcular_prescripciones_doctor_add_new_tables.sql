   ...
   BEGIN TRY
	...
	INSERT INTO prescripciones_doctor(doctor_id, fecha, codigo_laboratorio, nombre_laboratorio,codigo_producto, nombre_producto, marca, prescripciones)
    	SELECT DISTINCT [doctor].id AS doctor_id,
    		...
    		[Market_01_laboratorio].NOME AS nombre_laboratorio,
    		[01_ELE_prescricao].CDGPRO codigo_producto,
    		[Market_01_producto].NOME AS nombre_producto,
    		[Market_01_producto].MARCA AS marca,
    	...
    	LEFT JOIN [Market_01_laboratorio] ON TRIM([Market_01_laboratorio].CODIGO) = TRIM([01_ELE_prescricao].CDGLAB)
    	LEFT JOIN [Market_01_producto] ON TRIM([Market_01_producto].CODIGO) = TRIM([01_ELE_prescricao].CDGPRO)
    	...
    	WHERE [apm].inactivo = 0
    		...
    		AND [Market_01_producto].MARCA IS NOT NULL
    	ORDER BY
    			[doctor].id, fecha;
