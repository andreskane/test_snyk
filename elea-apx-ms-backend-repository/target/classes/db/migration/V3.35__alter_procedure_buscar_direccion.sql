...
BEGIN
	BEGIN TRY
		...
		--BEGIN
			INSERT INTO direccion (id_codigo_postal, numero, detalle, piso, departamento, otro)
			VALUES(@id_cp, @numero, @detalle, @piso, @departamento, @otros )
		--END
		COMMIT TRANSACTION T1
		/*SELECT top 1 @id_dir = id FROM direccion where id_codigo_postal = @id_cp and numero = @numero and detalle LIKE TRIM(@detalle)
		and (@piso is null or piso = @piso) and (@departamento is null or departamento = @departamento)
		and (@otros is null or otro = @otros)*/

		SELECT @id_dir = SCOPE_IDENTITY()
		END TRY
	BEGIN CATCH
