ALTER PROCEDURE [dbo].[Reporte_Aprobacion_Altas_Bajas] @ciclo int
AS
BEGIN
	SELECT
	...
	WHERE
	(cme.estado ='BAJA_APROBADA' AND cme.fecha_solicitud BETWEEN ci.inicio AND ci.fin)
	OR (((cme.estado = 'ALTA_APROBADA' OR cme.estado = 'ACTIVACION_APROBADA') AND cm.inactivo = 0) AND cme.fecha_solicitud BETWEEN ci.inicio AND ci.fin)
